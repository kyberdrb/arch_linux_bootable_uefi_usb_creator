...
DISK_NAME="$1"
ALREADY_DOWNLOADED_arch_linux_ISO="$2"

SCRIPT_DIR="$(dirname "$(readlink --canonicalize "$0")")"

"${SCRIPT_DIR}/utils/prepare_usb_for_uefi_booting.sh" "${DISK_NAME}"
"${SCRIPT_DIR}/utils/install_arch_linux_to_prepared_usb.sh" "${DISK_NAME}" 
...
echo
echo "Links to the complete arch_linux install script"
